...
namespace DepotService.Infraestructure.Documents
{
    ...
    public class InvoicedOrdersByCustomerDocument : IDocument
    {
        ...
        public void Compose(IDocumentContainer container)
        {
            ...
            if (!_orders.Any())
            {
                container.Page(page =>
                {
                    ...
                    page.Content()
                        .AlignMiddle()
                        .AlignCenter()
                        .Text("No existen pedidos facturados para este cliente.")
                        .FontSize(14)
                        .Bold()
            ...
            string customerName = _orders.First().CustomerName;
            decimal totalAmount = _orders.Sum(o => o.TotalAmount);
                ...
                col.Item().PaddingVertical(15).Row(row =>
                {
                    row.RelativeItem().Column(column =>
                    {
                        ...
                        column.Item().PaddingTop(5)
                            .Text($"Reporte de pedidos para: {customerName}")
                            .FontSize(13)
                            .FontColor(ColorNegro);

                        column.Item().PaddingTop(8).Text(text =>
                        {
                            text.Span("Fecha de generación: ")
        ...
        private void ComposeContent(IContainer container, string customerName, decimal totalAmount)
        {
            ...
            {
                col.Item().Element(content => ComposeTable(content, totalAmount));
            });
        }

        private void ComposeTable(IContainer container, decimal totalAmount)
        {
            container.Table(table =>
            {
                table.ColumnsDefinition(cols =>
                {
                    ...
                    cols.ConstantColumn(100);   // Cantidad
                ...
                foreach (var order in _orders)
                {
                    ...
                    table.Cell().Background(isEven ? Colors.White : ColorFondoTabla)
                        .BorderBottom(1).BorderColor("#DEDEDE")
                        .Padding(9).AlignCenter()
                        .Text(order.DepotOrderId.ToString())
                    ...
                    table.Cell().Background(isEven ? Colors.White : ColorFondoTabla)
                        .BorderBottom(1).BorderColor("#DEDEDE")
                        .Padding(9).AlignCenter()
                        .Text($"{totalUnits} {(totalUnits == 1 ? "unidad" : "unidades")}")
                        .FontSize(10).FontColor(ColorNegro);

                    table.Cell().Background(isEven ? Colors.White : ColorFondoTabla)
                        .BorderBottom(1).BorderColor("#DEDEDE")
                        .Padding(9).AlignRight()
