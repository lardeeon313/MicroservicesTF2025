...
namespace DepotService.Infraestructure.Messaging.Consumers.LogisticConsumers
{
    public class OrderDeliveryIncidentConsumer : BackgroundService
    {
        ...
        private const string QUEUE_NAME = "depot_order_delivery_incident_queue";
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                ...
                try
                {
                    ...
                    if (evento is not null)
                    {
                        ...
                        if (order != null)
                        {
                            ...
                            _logger.LogInformation("📦 Order {OrderId} marked as Delivered.", order.DepotOrderId);
                ...
                {
                    _logger.LogError(ex, "❌ Error processing DeliveryIncident order event {OrderId}", evento?.DepotOrderId ?? 0);
                    await _channel.BasicNackAsync(ea.DeliveryTag, false, requeue: false);
                }
            };
