...
namespace DepotService.Infraestructure.Messaging.Consumers.LogisticConsumers
{
    ...
    public class OrderAssignedDeliveryConsumer : BackgroundService
    {
        ...
        private const string QUEUE_NAME = "depot_order_assigned_delivery_queue";
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                ...
                try
                {
                    ...
                    if (evento is not null)
                    {
                        ...
                        if (order != null)
                        {
                            ...
                            _logger.LogInformation("📦 Order {OrderId} marked as AssignedDelivery.", order.DepotOrderId);
                ...
                {
                    _logger.LogError(ex, "❌ Error processing assigned delivery order event {OrderId}", evento?.DepotOrderId ?? 0);
                    await _channel.BasicNackAsync(ea.DeliveryTag, false, requeue: false);
                }
            };
