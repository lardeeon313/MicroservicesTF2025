...
export const DepotTeamPerformanceFilter: React.FC<Props> = ({ onSearch }) => {
  ...
  const calcularPeriodo = () => {
    ...
    switch (periodo) {
      case "hoy": desde = new Date(); break;
      case "semana": desde.setDate(hoy.getDate() - 7); break;
      case "quincena": desde.setDate(hoy.getDate() - 15); break;
      case "mes": desde.setMonth(hoy.getMonth() - 1); break;
    }

    const format = (d: Date) => d.toISOString().split("T")[0];

    return { from: format(desde), to: format(hoy) };
  };

  const handleSubmit = () => {
    const { from: f, to: t } = calcularPeriodo();
    onSearch({ from: f, to: t, agruparPorEquipo: agrupar });
  };
        ...
        <div className="flex items-center gap-3">
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className="w-11 h-6 bg-gray-300 rounded-full peer 
                          peer-checked:bg-red-600 transition-all"></div>
            <div className="absolute left-0.5 top-0.5 w-5 h-5 bg-white rounded-full 
                          shadow transition-all peer-checked:translate-x-5"></div>
          </label>
