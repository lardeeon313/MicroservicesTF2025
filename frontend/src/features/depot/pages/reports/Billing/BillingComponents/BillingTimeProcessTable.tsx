...
const BillingTimeProcessTable: React.FC<Props> = ({ data }) => {
  ...
  // Función para categorizar el tiempo de procesamiento
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            return (
              ...
              >
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    <div className="w-10 h-10 bg-gradient-to-br from-violet-400 to-purple-500 rounded-full flex items-center justify-center text-white font-medium text-sm mr-3">
                      #
                    </div>
                    <div>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${timeCategory.bg} ${timeCategory.color}`}>
                        {row.averageProcessingTime <= 15 ? 'Rápido' : 
                         row.averageProcessingTime <= 30 ? 'Normal' : 
                         row.averageProcessingTime <= 60 ? 'Lento' : 'Muy Lento'}
                      </div>
                    </div>
                  </div>
                </td>
      ...
      <div className="bg-gray-50 px-6 py-3 border-t border-gray-200">
        <p className="text-sm text-gray-600">
          Mostrando <span className="font-medium">{tableData.length}</span> pedido{tableData.length !== 1 ? 's' : ''} procesado{tableData.length !== 1 ? 's' : ''}
        </p>
      </div>
    </div>
  );
