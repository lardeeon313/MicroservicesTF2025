...
export const GraphPendingCashVerification: React.FC<Props> = ({ data }) => {
  ...
  const chartData = data.reduce((acc: any[], item) => {
    const date = new Date(item.orderDate).toLocaleDateString('es-AR', { month: 'short', year: 'numeric' });
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white px-4 py-3 rounded-lg shadow-xl border-2 border-blue-100">
          <p className="text-sm font-bold text-gray-800 mb-1">{payload[0].payload.date}</p>
          <p className="text-lg font-semibold text-blue-600">
            ${payload[0].value.toLocaleString('es-AR')}
          </p>
          <p className="text-xs text-gray-500 mt-1">
            {payload[0].payload.count} {payload[0].payload.count === 1 ? 'orden' : 'órdenes'}
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="w-full bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 shadow-sm">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-800">Verificación de Efectivo Pendiente</h3>
        <p className="text-sm text-gray-500">Montos por período</p>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 flex justify-around">
        <div className="text-center">
          ...
          <p className="text-lg font-bold text-gray-800">
            ${chartData.reduce((sum, item) => sum + item.amount, 0).toLocaleString('es-AR')}
          </p>
        </div>
        <div className="text-center">
          <p className="text-xs text-gray-500">Promedio</p>
          <p className="text-lg font-bold text-gray-800">
            ${Math.round(chartData.reduce((sum, item) => sum + item.amount, 0) / chartData.length).toLocaleString('es-AR')}
          </p>
        </div>
        <div className="text-center">
