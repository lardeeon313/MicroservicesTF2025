...
import { DepotTeamPerformanceFilter } from "../Filters/TeamProductivityFilters/TeamProductivityFilter";
import { useDepotTeamPerformance } from "../Hocks/TeamProducitity/useAdminTeamProductivity";
...
export const AdminReportDepotTeamPerformancePage: React.FC = () => {
  ...
  const [showGraph, setShowGraph] = useState(true); // Gráfico visible por defecto
        ...
        {!loading && data.length > 0 && (
          <div className="flex justify-end gap-3 mb-6">
            ...
            <button
              ...
              disabled={!lastFilters || loading}
              className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition-colors shadow-sm disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
        ...
        <div className="space-y-10">
          ...
          {!loading && data.length > 0 && (
            <>
              ...
              {showGraph && (
                <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200">
                  <AdminGraphTeamProductivity data={data} agruparPorEquipo={agrupar} />
                </div>
              )}
            </>
          )}
