...
{
    /// <summary>
    /// Consumidor para eventos de pedidos Assignados a Reparto.
    /// </summary>
    public class OrderAssignedDeliveryConsumer : BackgroundService
    {
        ...
        private const string EXCHANGE_NAME = "order_assigned_delivery_exchange";
        private const string QUEUE_NAME = "sales_order_assigned_delivery_queue";

        public OrderAssignedDeliveryConsumer(
            ILogger<OrderAssignedDeliveryConsumer> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    _logger.LogInformation("✅ Connected to RabbitMQ (OrderAssignedDeliveryConsumer).");
                    break;
                }
                catch (Exception ex)
                {
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                ...
                OrderAssignDeliveryIntegrationEvent? evento = null;

                try
                {
                    evento = JsonSerializer.Deserialize<OrderAssignDeliveryIntegrationEvent>(json);
                    ...
                    if (evento is not null)
                    {
                        ...
                        if (order != null)
                        {
                            order.Status = OrderStatus.AssignedDelivery;
                            order.ModifiedStatusDate = evento.AssignDeliveryAt;
                            ...
                            _logger.LogInformation("📦 Order {OrderId} marked as AssignedDelivery.", order.Id);
                ...
                {
                    _logger.LogError(ex, "❌ Error processing assigned delivery order event {OrderId}", evento?.SalesOrderId ?? 0);
                    await _channel.BasicNackAsync(ea.DeliveryTag, false, requeue: false);
                }
            };
            ...
            {
                _logger.LogInformation("OrderAssignedDeliveryConsumer stopping...");
            }
            finally
            {
