...
namespace IdentityService.Infraestructure.EmailTemplates
{
    public class MailgunEmailService : IEmailService
    {
        ...
        public async Task SendResetPasswordEmail(string email, string token)
        {
            ...
            // Construimos la URL que usará el frontend
            string url = $"http://localhost:3000/create-new-password?token={token}";

            string html = EmailTemplateGenerator.GenerateResetPasswordTemplate(url);
