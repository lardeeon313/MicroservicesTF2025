...
{
    public class GetMyDeliveredOrdersQueryHandler(ILogisticOrderRepository repository, ILogger<GetMyDeliveredOrdersQueryHandler> logger) : IGetMyDeliveredOrdersQueryHandler
    {
        private readonly ILogisticOrderRepository _repository = repository ?? throw new ArgumentNullException(nameof(repository));
        private readonly ILogger<GetMyDeliveredOrdersQueryHandler> _logger = logger ?? throw new ArgumentNullException(nameof(logger));

        /// <summary>
        /// handler para obtener los pedidos entregados por un operador de logística.
        /// </summary>
        /// <param name="query"></param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        public async Task<List<LogisticOrderDto>> GetMyDeliveredOrders(GetMyDeliveredOrdersQuery query)
        {
            var orders = await _repository.GetMyDeliveredOrders(query.OperatorId);
            if (orders == null || !orders.Any())
            {
                _logger.LogWarning("No delivered orders found for operator {OperatorId}", query.OperatorId);
                return new List<LogisticOrderDto>();
            }
            _logger.LogInformation("Found {Count} delivered orders for operator {OperatorId}", orders.Count(), query.OperatorId);
