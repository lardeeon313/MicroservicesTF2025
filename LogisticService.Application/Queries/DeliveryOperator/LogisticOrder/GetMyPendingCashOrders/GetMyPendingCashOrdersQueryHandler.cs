...
{
    public class GetMyPendingCashOrdersQueryHandler(ILogisticOrderRepository repository, ILogger<GetMyPendingCashOrdersQueryHandler> logger) : IGetMyPendingCashOrdersQueryHandler
    {
        private readonly ILogisticOrderRepository _repository = repository ?? throw new ArgumentNullException(nameof(repository));
        private readonly ILogger<GetMyPendingCashOrdersQueryHandler> _logger = logger ?? throw new ArgumentNullException(nameof(logger));

        /// <summary>
        /// handler para obtener los pedidos pendientes de pago en efectivo por un operador de logística.
        /// </summary>
        /// <param name="query"></param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        public async Task<List<LogisticOrderDto>> GetMyPendingCashOrders(GetMyPendingCashOrdersQuery query)
        {
            var orders = await _repository.GetMyPendingCashOrders(query.OperatorId);
            if (orders == null || !orders.Any())
            {
                _logger.LogWarning("No pending cash orders found for operator {OperatorId}", query.OperatorId);
                return new List<LogisticOrderDto>();
            }
            _logger.LogInformation("Found {Count} pending cash orders for operator {OperatorId}", orders.Count(), query.OperatorId);
