...
{
    public class GetMyAssignedOrdersQueryHandler(ILogisticOrderRepository repository, ILogger<GetMyAssignedOrdersQueryHandler> logger) : IGetMyAssignedOrdersQueryHandler
    {
        private readonly ILogisticOrderRepository _repository = repository ?? throw new ArgumentNullException(nameof(repository));
        private readonly ILogger<GetMyAssignedOrdersQueryHandler> _logger = logger ?? throw new ArgumentNullException(nameof(logger));

        /// <summary>
        /// Handler para obtener los pedidos asignados a un operador de logística.
        /// </summary>
        /// <param name="query"></param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        public async Task<List<LogisticOrderDto>> GetMyAssignedOrders(GetMyAssignedOrdersQuery query)
        {
            var orders = await _repository.GetMyAssignedOrders(query.OperatorId);
            if (orders == null || !orders.Any())
            {
                _logger.LogWarning("No orders found for operator {OperatorId}", query.OperatorId);
                return new List<LogisticOrderDto>();
            }
            _logger.LogInformation("Found {Count} orders for operator {OperatorId}", orders.Count(), query.OperatorId);
