...
{
    public class GetOrdersByDeliveryZoneIdQueryHandler(ILogisticOrderRepository repository, ILogger<GetOrdersByDeliveryZoneIdQueryHandler> logger) : IGetOrdersByDeliveryZoneIdQueryHandler
    {
        private readonly ILogisticOrderRepository _repository = repository;
        private readonly ILogger<GetOrdersByDeliveryZoneIdQueryHandler> _logger = logger;

        /// <summary>
        /// Handler para obtener órdenes logísticas por el ID de la zona de entrega.
        /// </summary>
        /// <param name="query"></param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        public async Task<IEnumerable<LogisticOrderDto>> HandleAsync(GetOrdersByDeliveryZoneIdQuery query)
        {
            var orders = await _repository.GetOrdersByDeliveryZoneId(query.DeliveryZoneId);
            if (!orders.Any())
            {
                _logger.LogWarning("No logistic orders found for delivery zone ID {DeliveryZoneId}", query.DeliveryZoneId);
                throw new KeyNotFoundException($"No logistic orders found for delivery zone ID {query.DeliveryZoneId}");
            }
