...
const ListofArmOrders = ({ order }: Props) => {
  const navigation = useNavigation<NativeStackNavigationProp<DepotStackParamList>>();
  
  //Navega a a la pantalla de detalle del pedido
  const handleSeeDetail = () => {
    ...
    return (
      <View style={{ backgroundColor: "#ffffff", padding: 20, borderRadius: 12, marginBottom: 16, shadowColor: "#000", elevation: 4 }}>
        <Text style={{ fontSize: 20, fontWeight: '700', marginBottom: 6 }}>Pedido#{order.id.toString()}</Text>
        <Text style={{ fontSize: 18, fontWeight: '700', marginBottom: 6 }}>
          Cliente: {order.customerFirstName} {order.customerLastName}
        </Text>
        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 20 }}>
          <TouchableOpacity style={{ backgroundColor: '#3B82F6', padding: 8, borderRadius: 8 }} onPress={handleSeeDetail}>
